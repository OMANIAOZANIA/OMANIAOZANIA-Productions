...
getgenv().UIToggleKey = getgenv().UIToggleKey or Enum.KeyCode.Z
...
function Hide()
	Debounce = true
	RayfieldLibrary:Notify({Title = "Interface Hidden", Content = "The interface has been hidden, you can unhide the interface by tapping Z", Duration = 7})
...
function RayfieldLibrary:CreateWindow(Settings)
	...
	local Window = {}
	function Window:CreateTab(Name,ImageAsset)
		local SDone = false
		...
		if ImageAsset then
			...
			TabButton.Image.Image = ImageAsset
		...
		function Tab:CreateKeybind(KeybindSettings)
			...
			UserInputService.InputBegan:Connect(function(input, processed)
				...
				if CheckingForKey then
					if input.KeyCode ~= Enum.KeyCode.Unknown and input.KeyCode ~= getgenv().UIToggleKey then
...
function RayfieldLibrary:GetGUIObject()
	return Rayfield
end
...
UserInputService.InputBegan:Connect(function(input, processed)
	if (input.KeyCode == getgenv().UIToggleKey and not processed) then
		if Debounce then return end
		if Hidden then
			Hidden = false
			Unhide()
